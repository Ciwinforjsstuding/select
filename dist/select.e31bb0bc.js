...
var Select = /*#__PURE__*/function () {
  function Select(selector, options) {
    ...
    _classPrivateMethodGet(this, _render, _render2).call(this); //render контетнта
  ...
  _createClass(Select, [{
    ...
    value: function clickHandler(event) {
      var type = event.target.dataset.type; // console.log("target", event.target);
    ...
    value: function select(id) {
      ...
      this.$el.querySelectorAll('[data-type="item"]').forEach(function (elem) {
        elem.classList.remove("selected");
      }); // чистим окраску выбранных элементов

      this.$el.querySelector("[data-id=\"".concat(id, "\"]")).classList.add("selected"); // строчка для окраски нашего выбранного элемента

      this.options.onSelect ? this.options.onSelect(this.current) : null; //вызов callbacka котрый мы получили из options
    }
  }, {
    key: "initialScrollBar",
    value: function initialScrollBar() {
      var _this = this;

      this.$scrollElem = document.querySelector("#scroll"); // получение дроп дауна

      this.$scrollElem.addEventListener("scroll", function () {
        // навешивание события прокрутки
        _this.$scrollElem.classList.add("scroll"); //добавление скроллбара
...
var _render2 = function _render2() {
  //функция для рендера контента selecta
...
var _setup2 = function _setup2() {
  ...
  this.$el.addEventListener("click", this.clickHandler); // this.
...
},{"./bundle-url":"../../../../usr/local/lib/node_modules/parcel/src/builtins/bundle-url.js"}],"select/style.scss":[function(require,module,exports) {
...
window.s = select;
},{"./select/select":"select/select.js","./select/style.scss":"select/style.scss"}],"../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
